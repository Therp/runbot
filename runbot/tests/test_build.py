...
class Test_Build(common.TransactionCase):
    ...
    def test_build_job_type_from_branch_default(self):
        """test build job stage is computed from branch default job_type"""
        build = self.Build.create({
            'branch_id': self.branch.id,
            'name': 'd0d0caca0000ffffffffffffffffffffffffffff',
        })
    ...
    def test_build_job_type_from_branch_none(self):
        """test build job stage is computed from branch"""
    ...
    def test_build_job_type_can_be_set(self):
        """test build job stage can be set to something different than the one on the branch"""
